...
Snake.prototype.getNextPos = function(){
	...
	this.pos.forEach(function(value){
        ...
        //如果数组中的两个数据相等，就说明下一个点在蛇身上里面能找到，代表撞到自己了
        	selfCollied = true;
        }
	});
	if(selfCollied){
...
Snake.prototype.strategies = {
	move:function(format){//这个参数用来决定要不要删除最后一个方块(蛇尾),当传了这个参数之后就表示要做的事情是吃
      //创建一个新的身体(在旧蛇头的位置)
    ...
    var newHead = new Square(this.head.x/sw+this.direction.x,this.head.y/sh+this.direction.y,'snakeHead');
      ...
      newHead.viewContent.style.transform = 'rotate('+this.direction.rotate+'deg)';//设置蛇头方向
      newHead.create(); 
...
function createFood(){
	...
	while(include){
		x = Math.round(Math.random()*(td-1));//0-29随机
		y = Math.round(Math.random()*(tr-1));
        
        snake.pos.forEach(function(value){
		  if(value[0].x!=x && value[1].y!=y){
		  	//这个条件成立说明现在随机出来的这个食物不在蛇身上 
			include = false;
		}
		})	
	}
	...
	food.pos = [x,y];//存储生成食物的坐标，用于跟蛇头要走的下个点做比较
...
Game.prototype.init = function (){
    ...
    document.onkeydown = function(ev){
    	if(ev.which==37 && snake.direction != snake.directionNum.right){//用户按下左键的时候，，这条蛇不能是正往右走
